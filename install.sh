...
wget --no-check-certificate "https://raw.githubusercontent.com/bayualhaq/sstidjg/main/sstidjg.sh" -O /usr/bin/sstidjg
wget --no-check-certificate "https://raw.githubusercontent.com/bayualhaq/sstidjg/main/autorekonek-sstidjg.sh" -O /usr/bin/autorekonek-sstidjg
wget --no-check-certificate "https://github.com/bayualhaq/sstidjg/blob/main/simple-obfs_0.0.5-5_aarch64_cortex-a53.ipk?raw=true" -O ~/simple-obfs.ipk
wget --no-check-certificate "https://github.com/bayualhaq/sstidjg/blob/main/ck-client?raw=true" -O /usr/bin/ck-client
opkg install ip-full && opkg install *.ipk && opkg install lsof && opkg install fping
chmod +x /usr/bin/sstidjg
chmod +x /usr/bin/autorekonek-sstidjg
chmod +x /usr/bin/ck-client
rm -r ~/*.ipk
rm -r ~/install.sh
mkdir -p ~/akun/
touch ~/akun/sstidjg.conf
touch ~/akun/sstidjg.txt
sleep 2
echo "install selesai"
echo "untuk memulai tools silahkan jalankan perintah 'sstidjg'"
