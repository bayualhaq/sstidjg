...
#sstidjg (Wegare)
clear
udp2="$(cat /root/akun/sstidjg.txt | grep -i udp | cut -d= -f2)" 
host2="$(cat /root/akun/sstidjg.txt | grep -i host | cut -d= -f2 | head -n1)" 
port2="$(cat /root/akun/sstidjg.txt | grep -i port | cut -d= -f2)" 
bug2="$(cat /root/akun/sstidjg.txt | grep -i bug | cut -d= -f2)" 
pass2="$(cat /root/akun/sstidjg.txt | grep -i pass | cut -d= -f2)" 
enc2="$(cat /root/akun/sstidjg.txt | grep -i enc | cut -d= -f2)" 
uid2="$(cat /root/akun/sstidjg.txt | grep -i uid | cut -d: -f2)" 
publickey2="$(cat /root/akun/sstidjg.txt | grep -i publickey | cut -d: -f2)" 
plugin2="$(cat /root/akun/sstidjg.txt | grep -i plugin | cut -d= -f2)" 
obfs2="$(cat /root/akun/sstidjg.txt | grep -i obfs | cut -d= -f2 | tail -n1)" 
...
cat <<EOF> /root/akun/sstidjg.json
{
  "server" : "$host",
  "server_port" : "$port",
  "method" : "$enc",
...
plugin=$plugin
obfs=$obfs
udp=$udp" > /root/akun/sstidjg.txt

cat <<EOF> /usr/bin/gproxy-sstidjg
...
EOF
chmod +x /usr/bin/gproxy-sstidjg
...
udp="$(cat /root/akun/sstidjg.txt | grep -i udp | cut -d= -f2)" 
host="$(cat /root/akun/sstidjg.txt | grep -i host | cut -d= -f2 | head -n1)" 
route="$(cat /root/akun/ipmodem.txt | grep -i ipmodem | cut -d= -f2 | tail -n1)" 

ss-local -c /root/akun/sstidjg.json &
sleep 3
ip tuntap add dev tun1 mode tun
ifconfig tun1 10.0.0.1 netmask 255.255.255.0
/usr/bin/gproxy-sstidjg
...
#sstidjg (Wegare)
host="$(cat /root/akun/sstidjg.txt | grep -i host | cut -d= -f2 | head -n1)"
fping -l $host' > /usr/bin/ping-sstidjg
chmod +x /usr/bin/ping-sstidjg
/usr/bin/ping-sstidjg > /dev/null 2>&1 &
elif [ "${tools}" = "3" ]; then
host="$(cat /root/akun/sstidjg.txt | grep -i host | cut -d= -f2 | head -n1)" 
route="$(cat /root/akun/ipmodem.txt | grep -i ipmodem | cut -d= -f2 | tail -n1)" 
#killall screen
killall -q badvpn-tun2socks ss-local ping-sstidjg fping
...
# BEGIN AUTOREKONEKsstidjg
*/1 * * * *  autorekonek-sstidjg
# END AUTOREKONEKsstidjg
EOF
...
sed -i "/^# BEGIN AUTOREKONEKsstidjg/,/^# END AUTOREKONEKsstidjg/d" /etc/crontabs/root > /dev/null
/etc/init.d/cron restart
